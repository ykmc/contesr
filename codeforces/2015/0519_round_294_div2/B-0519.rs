...
// Module
use std::cmp::{min,max};
use std::collections::{VecDeque,HashSet,HashMap,BTreeSet,BTreeMap,BinaryHeap};
// Module for v1.15.1
...
fn main() {
    ...
    for i in 0..N {
        let cnt = map_a.entry(A[i]).or_insert(0);
        *cnt += 1;
    }
    for i in 0..N-1 {
        let cnt = map_b.entry(B[i]).or_insert(0);
        *cnt += 1;
    }
    for i in 0..N-2 {
        let cnt = map_c.entry(C[i]).or_insert(0);
        *cnt += 1;
    }
    ...
    let mut ans2 = 0;
    for s in set {
        if ans1==0 && map_a.get(&s) != map_b.get(&s){
            ans1 = s;
        }
        if ans2==0 && map_b.get(&s) != map_c.get(&s){
            ans2 = s;
        }
    }
